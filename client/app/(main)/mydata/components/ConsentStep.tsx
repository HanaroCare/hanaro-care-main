...
export default function ConsentStep({ onNext }: ConsentStepProps) {
  ...
  const isRequiredAgreed = agreements.terms && agreements.privacy && agreements.mydata;
      ...
      <div className="flex flex-col gap-[1.5rem]">
        <button
          ...
          className={`flex items-center gap-[0.75rem] rounded-[1rem] p-[1.25rem] transition-colors ${
            agreements.all ? "bg-primary/5 border border-primary/20" : "bg-gray-50 border border-transparent"
          }`}
        >
          <div className={`flex h-[1.5rem] w-[1.5rem] items-center justify-center rounded-full ${
            agreements.all ? "bg-primary text-white" : "bg-gray-200 text-gray-400"
          }`}>
            <Check size={16} strokeWidth={3} />
          </div>
          <span className="text-[1.125rem] font-bold text-foreground">전체 동의하기</span>
        </button>
        ...
        <div className="flex flex-col gap-[1rem] px-[0.5rem]">
          {[
            ...
            <div key={item.key} className="flex items-center justify-between">
              <button
                type="button"
                onClick={() => handleToggle(item.key as keyof typeof agreements)}
                className="flex items-center gap-[0.75rem]"
              >
                <Check 
                  size={20} 
                  className={agreements[item.key as keyof typeof agreements] ? "text-primary" : "text-gray-300"} 
                />
                <span className="text-[1rem] text-hana-black-700">{item.label}</span>
              </button>
              <button 
                type="button" 
                onClick={() => setSelectedTerms(TERMS_DATA[item.key as keyof typeof TERMS_DATA])}
