...
const maskId = (id: string): string => {
  if (!id) return "";
  const len = id.length;
  if (len <= 3) return id.charAt(0) + "*".repeat(len - 1);
  const visibleStart = 3;
  const visibleEnd = 2;
  if (len <= visibleStart + visibleEnd) return id.substring(0, 2) + "*".repeat(len - 2);
...
function FindIdResultContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const foundId = searchParams.get("loginId") ?? "";
