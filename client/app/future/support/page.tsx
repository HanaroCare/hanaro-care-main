...
const supportItems: SupportItem[] = [
  {
    ...
    title: "돌봄플러스 케어",
    description:
      "65세 이상 독거노인 대상 맞춤형 돌봄 서비스입니다. 주 3회 방문 케어를 제공합니다.",
    tab: "local",
  },
  {
    id: 2,
        ...
        >
          {filtered.map((item) => (
            <button
              key={item.id}
              onClick={() =>
                router.push(`/future/support/detail/${item.id}` as Route)
              }
            ...
            >
              <div className="flex flex-col gap-[4px] flex-1 pr-[10px]">
                <span className="font-medium text-[18px] leading-[22px] text-[#1A212D]">
                  {item.title}
                </span>
