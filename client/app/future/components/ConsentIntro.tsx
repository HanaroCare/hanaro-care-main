...
export default function ConsentIntro({
  ...
  return (
    <div className="relative w-full min-h-screen bg-white flex flex-col">
      {/* 스크롤 컨테이너 */}
      <div className="flex flex-col flex-1 overflow-y-auto pb-[100px] pt-[65px]">
        {/* 동영상 배너 */}
        <div
        ...
        >
          ...
          <div className="flex flex-col items-center gap-0.5">
            <span className="font-medium text-[14px] leading-[21px] text-white">{videoSubtitle}</span>
            <span className="font-medium text-[16px] leading-[24px] text-white">{videoTitle}</span>
          </div>
        </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 flex flex-row gap-[7px] px-[25px] pb-[30px] bg-white">
        ...
        >
          ...
          <span className="font-semibold text-[14px] leading-[24px] text-white">{applyLabel}</span>
        </button>
      </div>
    </div>
  );
