...
export default function DevAdminPage() {
  ...
  const [trustResult, setTrustResult] = useState<ResultState>({ status: 'idle', message: '' });

  const [pensionUserId, setPensionUserId] = useState('');
  const [pensionRealAssetId, setPensionRealAssetId] = useState('');
  const [pensionResult, setPensionResult] = useState<ResultState>({ status: 'idle', message: '' });

  const [agentUserId, setAgentUserId] = useState('');
  const [agentResult, setAgentResult] = useState<ResultState>({ status: 'idle', message: '' });
  ...
  const handleTrustSubscribe = () => {
    ...
    startTransition(async () => {
      try {
        ...
        setTrustResult({ status: 'success', message: `가입 완료 — userProdId: ${userProdId}` });
      } catch (e) {
        setTrustResult({ status: 'error', message: `실패: ${e instanceof Error ? e.message : String(e)}` });
      }
    });
  };
  ...
  const handlePensionSubscribe = () => {
    ...
    startTransition(async () => {
      try {
        const { userProdId } = await subscribePensionProduct(userId, realAssetId);
        setPensionResult({ status: 'success', message: `가입 완료 — userProdId: ${userProdId}` });
      } catch (e) {
        setPensionResult({ status: 'error', message: `실패: ${e instanceof Error ? e.message : String(e)}` });
      }
    });
  };
  ...
  const handleEnableAgentView = () => {
    ...
    startTransition(async () => {
      try {
        ...
        setAgentResult({ status: 'success', message: '대리인 열람 권한 허용 완료' });
      } catch (e) {
        setAgentResult({ status: 'error', message: `실패: ${e instanceof Error ? e.message : String(e)}` });
      }
    });
  };
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] px-4 py-8">
      <div className="mx-auto max-w-md">
        <div className="mb-6">
          <p className="font-bold text-[20px] text-[#1F2937]">관리자 테스트 패널</p>
