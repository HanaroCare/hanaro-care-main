...
public class TrustService {
	...
	public TrustProductResponse getProductSummary(Long userId, Long userProdId) {
		...
		BigDecimal currentAmount   = principalAmount.add(profit);

		TrustSimulationSaveRequest.PayoutSettingsDto payoutSettings =
			parsePayoutSettings(userProd.getPayoutSettings());
		TrustProductResponse.ExecutionSetting executionSetting =
			mapExecutionSetting(payoutSettings.items());

		BigDecimal monthlyExecutionAmount =
			TrustCalculator.defaultIfNull(executionSetting.hospitalAmount())
				.add(TrustCalculator.defaultIfNull(executionSetting.livingAmount()));

		LocalDate startDate  = userProd.getCreatedAt().toLocalDate();
		long monthsPassed = Math.max(
			ChronoUnit.MONTHS.between(YearMonth.from(startDate), YearMonth.from(LocalDate.now())) + 1,
			0
		);

		BigDecimal executionAmount = monthlyExecutionAmount.multiply(BigDecimal.valueOf(monthsPassed));
	...
	private TrustProductResponse.ExecutionSetting mapExecutionSetting(
		...
		return new TrustProductResponse.ExecutionSetting(
			hospitalEnabled, hospitalAmount,
			livingEnabled, livingAmount
		);
	}

	@Transactional
	public void updatePayoutSettings(
		Long userId,
		Long userProdId,
		TrustPayoutSettingsUpdateRequest request
	) {
		TBUserProd userProd = userProdRepository.findById(userProdId)
			.orElseThrow(() -> new ApiException(ErrorStatus.PRODUCT_NOT_FOUND));
		...
		try {
			String payoutSettingsJson = objectMapper.writeValueAsString(request);
			userProd.setPayoutSettings(payoutSettingsJson);
		} catch (Exception e) {
	...
	public void updateAgentView(
		Long userId,
		Long userProdId,
		TrustAgentViewUpdateRequest request
	) {
		TBUserProd userProd = userProdRepository.findById(userProdId)
			.orElseThrow(() -> new ApiException(ErrorStatus.PRODUCT_NOT_FOUND));
	...
	private void validateOwner(Long userId, TBUserProd userProd) {
		if (!userProd.getUser().getUserId().equals(userId)) {
